...
**Optional hero image** — if image generation is available (check surf-cli, then `$GEMINI_API_KEY` -- see SKILL.md), generate a hero banner with `--aspect-ratio 16:9` that visually captures the project's identity or domain. Match the style to the page's palette. Embed as base64 data URI using the `.hero-img-wrap` pattern from css-patterns.md. Place above or just below the title. Skip if neither is available -- the page should stand on its own.
