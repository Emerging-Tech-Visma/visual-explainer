...
**Optional illustrations** — if image generation is available (check surf-cli, then `$GEMINI_API_KEY` -- see SKILL.md), consider generating a conceptual illustration of the planned system when it would help the reader visualize the change. Embed as base64 data URI. See css-patterns.md "Generated Images" for container styles. Skip if neither is available or the plan is purely structural.
