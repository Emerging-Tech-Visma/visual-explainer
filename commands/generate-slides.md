...
Follow the visual-explainer skill workflow. Read the reference template at `${CLAUDE_SKILL_DIR}/templates/slide-deck.html` and slide patterns at `${CLAUDE_SKILL_DIR}/references/slide-patterns.md` before generating. Also read `${CLAUDE_SKILL_DIR}/references/css-patterns.md` for shared patterns (Mermaid zoom controls, depth tiers, overflow protection) and `${CLAUDE_SKILL_DIR}/references/libraries.md` for Mermaid theming, Chart.js, and font pairings.
...
**Visual richness:** Proactively reach for visuals. If image generation is available (check surf-cli, then `$GEMINI_API_KEY` -- see SKILL.md), generate images for title slide backgrounds and full-bleed slides. Add SVG decorative accents, inline sparklines, mini-charts, and small Mermaid diagrams where they make the story more compelling. Visual-first, text-second.
